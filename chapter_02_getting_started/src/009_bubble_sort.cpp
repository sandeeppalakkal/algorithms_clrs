...
Starts from the end of the array. Scans through the first element. Considers
every successive pair of numbers at a time and swaps them if necessary to
correct the order. This procedure is repated again from the end of the array
through the second element, again through the third element and so on. Each
time, the minimum number on the right sub-array is moved to the beginning of the
subarray. This results in sorting.

Bubble sort is slower than insertion sort and is only fast when the array is
partially sorted.
...
/**
  ...
  */
void bubble_sort(Dtype *array, const int array_len, const SORT_TYPE sort_type)
{
   for(int i = 0; i < array_len - 1; ++i)
   {
       for(int j = array_len - 1; j > i; --j)
       {
           if(compare(array[j-1], array[j], sort_type))
           {
               int temp = array[j];
               array[j] = array[j - 1];
               array[j - 1] = temp;
...
int main(int argc, char **argv)
{
    ...
    clock_t time = clock();
    bubble_sort(array, array_len, sort_type);
    time = clock() - time;
